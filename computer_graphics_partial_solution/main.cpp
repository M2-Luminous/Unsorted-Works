...
#include <C:/Users/M2-Winterfell/Graphics/Solar-System-master/libs/glm/glm.hpp>
#include <C:/Users/M2-Winterfell/Graphics/Solar-System-master/libs/glm/gtc/matrix_transform.hpp>
#include <C:/Users/M2-Winterfell/Graphics/Solar-System-master/libs/glm/gtc/type_ptr.hpp>
...
#define MESH_NAME "./assets/models/carcass_eater.dae"
...
// Textures
const char *sun = "./assets/textures/eater.jpg";
	...
	for (unsigned int m_i = 0; m_i < scene->mNumMeshes; m_i++) {
		...
		for (unsigned int v_i = 0; v_i < mesh->mNumVertices; v_i++) {
			...
			if (mesh->HasTextureCoords(0)) {
				...
				modelData.mTextureCoords.push_back(vec2(1.0f - vt->y, 1.0f - vt->x));
			}
...
void updateScene() {
	...
	if (keyW) camera += cameraSpeed * cameraFront;
	if (keyS) camera -= cameraSpeed * cameraFront;
	if (keyA) camera -= glm::normalize(glm::cross(cameraFront, up)) * cameraSpeed;
	if (keyD) camera += glm::normalize(glm::cross(cameraFront, up)) * cameraSpeed;
	if (keyQ) camera.y += cameraSpeed; // Move up
	if (keyE) camera.y -= cameraSpeed; // Move down

	// Diagonal movement
	if (keyW && keyA) camera += glm::normalize(glm::cross(up, cameraFront)) * cameraSpeed;
	if (keyW && keyD) camera -= glm::normalize(glm::cross(up, cameraFront)) * cameraSpeed;
	if (keyS && keyA) camera -= glm::normalize(glm::cross(cameraFront, up)) * cameraSpeed;
	if (keyS && keyD) camera += glm::normalize(glm::cross(cameraFront, up)) * cameraSpeed;
...
int main(int argc, char** argv) {
	...
	lastX = width / 2.0f; // Set lastX to the horizontal window center
	lastY = height / 2.0f; // Set lastY to the vertical window center
