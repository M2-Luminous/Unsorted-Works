...
public class MineSweeper {
	...
	public static void main(String[] args) {
		...
		System.out.println("After setting no. of mine neighbours:");
		printBoard(board);
		}
	...
	public static int numberOfNeighbouringMines( char[][] board, int passedRow, int passedColumn )
	{
		...
		for (int row=(passedRow==0)?0:passedRow-1; row <= ((passedRow==board.length-1)?passedRow:passedRow+1); row++)
			...
			for (int column=(passedColumn==0)?0:passedColumn-1;
			
				column <= ((passedColumn==board[row].length-1) ? passedColumn:passedColumn+1); column++)
